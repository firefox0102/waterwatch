...
function getColiformNumbers(report) {
  if (
    !report.coliformLargeCells ||
    !report.coliformSmallCells ||
    !report.totalColiform
  ) {
    return "";
  }
...
app.post("/export", (request, response) => {
  ...
  getCSVData({ exportType, startDate, endDate, collectionSites }).then(csv => {
